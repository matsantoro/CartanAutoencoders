...
from geoopt.optim import RiemannianAdam
torch.nn.utils.clip_grad_norm_(chain(hyp_encoder.parameters(), hyp_decoder.parameters()), max_norm=1.0)
...
for item, _ in list(iter(test_dataloader)):
    for image in range(10):
        ...
        axes[1].imshow(hyp_decoder(hyp_encoder(item.flatten(start_dim=1).cuda()))[image, 1:].reshape(28,28).cpu().detach().numpy())
        if not image:
